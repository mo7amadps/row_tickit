...
async function cacheGuildInvites(guild) {
  ...
  if (!me?.permissions.has(PermissionFlagsBits.ManageGuild)) {
    console.error(`❌ ما قدرت أجيب دعوات السيرفر ${guild.name}: البوت يحتاج صلاحية Manage Server.`);
    return false;
  }
  ...
  try {
    ...
    console.error(`❌ ما قدرت أجيب دعوات السيرفر ${guild.name}:`, e.message || e);
    return false;
  }
}
...
async function handleGuildMemberAdd(member, client) {
  ...
  if (usedInvite && usedInvite.inviter) {
    ...
    if (client.seenMembers.get(member.id)) {
      if (channel) await channel.send(`⚠️ <@${inviter.id}> هذا العضو <@${member.id}> سبق دخل السيرفر من قبل، لن تحصل على نقاط.`).catch(() => {});
    } else {
      // 2) هل عمر حساب العضو أقل من أسبوعين ونص؟ (حساب وهمي/جديد) => ما يُحسب
      const accountAge = Date.now() - member.user.createdTimestamp;
      if (accountAge < MIN_ACCOUNT_AGE_MS) {
        if (channel) await channel.send(`⚠️ <@${inviter.id}> حساب العضو <@${member.id}> عمره أقل من أسبوعين ونص (حساب جديد/مشتبه به)، لن تحصل على نقاط.`).catch(() => {});
      } else {
        ...
        if (channel) await channel.send(`✅ <@${inviter.id}> دعوت <@${member.id}> إلى السيرفر! نقاطك الآن: ${currentPoints + 1} 🔥`).catch(() => {});
      }
...
async function handlePrefixCommand(message, client) {
  ...
  if (message.content.startsWith('+add-points')) {
    ...
    const pointsToAdd = Number.parseInt(args[2], 10);
    if (!member || !Number.isSafeInteger(pointsToAdd) || pointsToAdd < 0 || pointsToAdd > 1_000_000) {
      return message.reply('❌ صيغة الأمر غير صحيحة. استخدم: `+add-points @mentionUser عدد_النقاط` (من 0 إلى 1,000,000)');
    }
  ...
  if (message.content === '+points' || message.content.startsWith('+points ')) {
...
async function handleButton(interaction, client) {
  ...
  const spinKey = `${interaction.guild.id}:${interaction.user.id}`;
  if (spinLocks.has(spinKey)) {
    await interaction.reply({ content: '⏳ جاري معالجة اللفة السابقة، حاول بعد لحظة.', flags: MessageFlags.Ephemeral });
    return true;
  }
  spinLocks.add(spinKey);

  try {
    if (interaction.customId === 'normal_spin') {
      if (userPointsCount < 1) {
        await interaction.reply({ content: '❌ ليس لديك نقاط كافية.', flags: MessageFlags.Ephemeral });
        return true;
      }
      client.invitePoints.set(interaction.user.id, userPointsCount - 1);
      const prize = getRandomPrize('normal');
      if (prizeChannel) {
        await prizeChannel.send(`> 🥳 مبروك <@${interaction.user.id}>! لقد فزت بـ **${prize}** 🏆`).catch(error => {
          console.error('❌ تعذر إرسال إعلان الجائزة:', error.message || error);
        });
      }
