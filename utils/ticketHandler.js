const {
  ...
  AttachmentBuilder, ChannelSelectMenuBuilder, MessageFlags
} = require('discord.js');
const fs = require('fs');
...
async function closeTicket(interaction, client, ticket, settings, stars) {
  const closeKey = `${interaction.guild.id}:${interaction.channel.id}`;
  if (closingTickets.has(closeKey)) return;
  closingTickets.add(closeKey);
  try {
    await closeTicketInternal(interaction, client, ticket, settings, stars);
  } finally {
    closingTickets.delete(closeKey);
  }
}

async function closeTicketInternal(interaction, client, ticket, settings, stars) {
  ...
  if (settings.ticket_logs) {
    ...
    if (logsChannel) {
      ...
      try {
        ...
        console.error('❌ تعذر إرسال لوق التذكرة:', error.message || error);
        ...
        if (transcriptPath) {
          try {
            ...
            console.error('❌ تعذر حذف ملف transcript المؤقت:', error.message || error);
          }
        }
      }
    }
...
async function handleModal(interaction, client) {
  ...
  if (interaction.customId === 'panel_embed_info') {
    ...
    await interaction.reply({ content: '**صورة الايمبد على اليمين**', components: [row], flags: MessageFlags.Ephemeral });
  }
  ...
  if (interaction.customId === 'ticket_add_user_modal') {
    ...
    if (!ticket) return interaction.reply({ content: '❌ هذا ليس شانل تذكرة!', flags: MessageFlags.Ephemeral });

    const member = await interaction.guild.members.fetch(userId).catch(() => null);
    if (!member) return interaction.reply({ content: '❌ المستخدم غير موجود، تأكد من الـ ID.', flags: MessageFlags.Ephemeral });
  ...
  if (interaction.customId === 'ticket_rename_modal') {
    ...
    if (!newName) return interaction.reply({ content: '❌ اسم غير صالح!', flags: MessageFlags.Ephemeral });
...
async function handleSelect(interaction, client) {
  ...
  if (interaction.customId.startsWith('panel_color_')) {
    ...
    await interaction.followUp({ content: `**اكتب اسم التذكرة رقم ${ticketNum}**`, flags: MessageFlags.Ephemeral });
  ...
  if (interaction.customId === 'ticket_menu_select') {
    ...
    if (interaction.values[0] === 'panel_refresh') {
      return await interaction.reply({ content: '🔄 البانل شغال! اختر نوع التذكرة من المنيو.', flags: MessageFlags.Ephemeral });
    }
...
async function handleButton(interaction, client) {
  ...
  if (interaction.customId === 'panel_refresh') {
    await interaction.reply({ content: '🔄 البانل شغال! اختر نوع التذكرة من فوق.', flags: MessageFlags.Ephemeral });
    return;
  }
  ...
  if (interaction.customId.startsWith('rate_')) {
    ...
    const ticket = client.openTickets.get(interaction.channelId);
    if (!ticket) return interaction.reply({ content: '❌ خطأ في التذكرة!', flags: MessageFlags.Ephemeral });

    // بس صاحب التذكرة يقدر يقيّم
    if (interaction.user.id !== ticket.userId) {
      return interaction.reply({ content: '❌ بس صاحب التذكرة يقدر يقيّم!', flags: MessageFlags.Ephemeral });
    }
  ...
  if (interaction.customId === 'ticket_close_skip') {
    ...
    if (!hasAdmin && !hasRole && !isOwner) {
      return interaction.reply({ content: '❌ بس الإداريين يقدرون يسكرون بدون تقييم!', flags: MessageFlags.Ephemeral });
    }
  ...
  if (interaction.customId === 'ticket_close') {
    ...
    // بس مستلم التذكرة أو الادمن يقدر يسكر من الزر
    const closerMember = await interaction.guild.members.fetch(interaction.user.id).catch(() => null);
    const closerHasAdmin = settings.admin && closerMember?.roles.cache.has(settings.admin);
    const closerIsOwner = interaction.guild.ownerId === interaction.user.id;
    const closerIsClaimer = ticket.claimedBy && interaction.user.id === ticket.claimedBy;

    if (!closerIsClaimer && !closerHasAdmin && !closerIsOwner) {
      return interaction.reply({ content: '❌ بس مستلم التذكرة أو الإداري يقدر يسكر التذكرة!', flags: MessageFlags.Ephemeral });
    }
  ...
  if (interaction.customId === 'ticket_claim') {
    if (!ticket) return interaction.reply({ content: '❌ هذا ليس شانل تذكرة!', flags: MessageFlags.Ephemeral });
    if (ticket.claimedBy) return interaction.reply({ content: '❌ التذكرة مستلمة بالفعل!', flags: MessageFlags.Ephemeral });

    // صاحب التذكرة ما يقدر يستلم تذكرته أبداً، حتى لو كان إداري
    if (interaction.user.id === ticket.userId) {
      return interaction.reply({ content: '❌ ما تقدر تستلم تذكرتك أنت!', flags: MessageFlags.Ephemeral });
    }
    ...
    if (!hasRole && !hasAdmin && !isOwner) {
      return interaction.reply({ content: '❌ بس المسؤولين يقدرون يستلمون التذاكر!', flags: MessageFlags.Ephemeral });
    }
  ...
  if (interaction.customId === 'ticket_unclaim') {
    ...
    if (ticket.claimedBy !== interaction.user.id) {
      return interaction.reply({ content: '❌ أنت لم تستلم هذه التذكرة!', flags: MessageFlags.Ephemeral });
    }
  ...
  if (interaction.customId === 'ticket_add_user') {
    ...
    if (!hasAdmin && !isClaimer) {
      return interaction.reply({ content: '❌ بس مستلم التذكرة أو الإداريين يقدرون يضيفون أشخاص!', flags: MessageFlags.Ephemeral });
    }
  ...
  if (interaction.customId === 'ticket_rename') {
    ...
    if (!hasAdmin && !isClaimer) {
      return interaction.reply({ content: '❌ بس مستلم التذكرة أو الإداريين يقدرون يغيرون اسم التذكرة!', flags: MessageFlags.Ephemeral });
    }
  ...
  if (interaction.customId === 'ticket_call_support') {
    ...
    if (!hasAdmin && !isClaimer) {
      return interaction.reply({ content: '❌ بس مستلم التذكرة أو الإداريين يقدرون يستخدمون هذا الزر!', flags: MessageFlags.Ephemeral });
    }

    const cdMsg = checkCooldown(interaction.user.id, 'call_support', interaction.channelId);
    if (cdMsg) return interaction.reply({ content: cdMsg, flags: MessageFlags.Ephemeral });
  ...
  if (interaction.customId === 'ticket_dm_user') {
    ...
    if (!ticketUser) return interaction.reply({ content: '❌ ما قدرت أجد العضو.', flags: MessageFlags.Ephemeral });

    const sent = await ticketUser.send({
      embeds: [{ description: `📩 **يرجى التوجه للتذكرة التي قمت بفتحها**\n\n🔗 ${interaction.channel}`, color: 0x5865F2 }]
    }).catch(() => null);

    if (!sent) return interaction.reply({ content: '❌ ما قدرت أرسل للعضو، ربما أغلق الخاص.', flags: MessageFlags.Ephemeral });
    await interaction.reply({ content: `✅ تم إرسال رسالة خاصة لـ ${ticketUser.tag}`, flags: MessageFlags.Ephemeral });
    return;
  }
}
...
async function sendPanel(interaction, client, session) {
  ...
  if (!channel) return interaction.followUp({ content: '❌ الروم غير موجود', flags: MessageFlags.Ephemeral });
  ...
  await interaction.followUp({ content: `✅ تم إرسال البانل إلى ${channel}`, flags: MessageFlags.Ephemeral });
}
...
async function openTicket(interaction, client, ticketNum, reason) {
  ...
  if (existingTicket) return interaction.reply({ content: `❌ لديك تذكرة بالفعل`, flags: MessageFlags.Ephemeral });
  ...
  client.openTickets.set(ticketChannel.id, {
    userId: user.id,
    ticketNum,
    guildId: guild.id,
    claimedBy: null,
  ...
  await interaction.reply({ content: `✅ تم فتح تذكرتك: ${ticketChannel}`, flags: MessageFlags.Ephemeral });
}

// بعد restart قد تكون بعض السجلات تشير إلى قنوات حذفها Discord. ننظف السجل
// الميت فقط؛ التكت الموجودة لا تُغلق تلقائياً، وتبقى قابلة للإغلاق بـ !close.
async function reconcileOpenTickets(client) {
  for (const [channelId, ticket] of [...client.openTickets.entries()]) {
    const channel = await client.channels.fetch(channelId).catch(() => null);
    if (!channel || !channel.guild) {
      client.openTickets.delete(channelId);
      console.log(`🧹 تمت إزالة سجل تذكرة بدون قناة: ${channelId}`);
      continue;
    }
...
async function handlePrefixCommand(message, client) {
  ...
  if (content === '!close') {
    ...
    const hasAdmin = settings.admin && member?.roles.cache.has(settings.admin);
    const isClaimer = ticket.claimedBy === message.author.id;
    const isOwner = message.guild.ownerId === message.author.id;
    if (!hasAdmin && !isClaimer && !isOwner) {
      return message.reply({ content: '❌ بس مستلم التذكرة أو الإداري يقدر يسكرها!' });
    }
