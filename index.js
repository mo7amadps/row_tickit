...
async function registerCommands() {
  ...
  for (const guild of guilds) {
    ...
    if (await pointsHandler.cacheGuildInvites(guild)) inviteCachesLoaded++;
  }

  await rest.put(Routes.applicationCommands(client.user.id), { body: [] });
  console.log(`✅ Registered ${commandsData.length} slash commands in ${guilds.length} server(s)`);
  console.log(`ℹ️ تم حفظ الدعوات في ${inviteCachesLoaded}/${guilds.length} سيرفر. إذا كان العدد أقل، راجع صلاحية Manage Server.`);
}
...
client.once('clientReady', async () => {
  ...
  try {
    ...
    console.error('❌ فشل تجهيز البوت بعد تسجيل الدخول:', error);
  } finally {
    startMaintenance();
  }
});
...
client.on('guildMemberAdd', async member => {
  try {
    await pointsHandler.handleGuildMemberAdd(member, client);
  } catch (error) {
    console.error('❌ خطأ أثناء معالجة دخول عضو جديد:', error);
  }
});
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isButton()) {
      const handled = await pointsHandler.handleButton(interaction, client);
      if (handled) return;
    ...
    console.error('❌ خطأ أثناء معالجة تفاعل Discord:', error);
...
client.on('messageCreate', async message => {
  try {
    ...
    await pointsHandler.handlePrefixCommand(message, client);
  } catch (error) {
